...
from .messages_pb2 import CoinsecdMessage


MAX_MESSAGE_LENGTH = 1024 * 1024 * 1024  # 1GB


class CoinsecdCommunicationError(Exception): pass


# pipenv run python -m grpc_tools.protoc -I./protos --python_out=. --grpc_python_out=. ./protos/rpc.proto ./protos/messages.proto ./protos/p2p.proto

class CoinsecdThread(object):
    def __init__(self, coinsecd_host, coinsecd_port, async_thread=True):

        self.coinsecd_host = coinsecd_host
        self.coinsecd_port = coinsecd_port

        if async_thread:
            self.channel = grpc.aio.insecure_channel(f'{coinsecd_host}:{coinsecd_port}',
            ...
            self.channel = grpc.insecure_channel(f'{coinsecd_host}:{coinsecd_port}',
                                                 compression=grpc.Compression.Gzip,
                                                 options=[
                                                     ('grpc.max_send_message_length', MAX_MESSAGE_LENGTH),
                                                     ('grpc.max_receive_message_length', MAX_MESSAGE_LENGTH),
    ...
    async def request(self, command, params=None, wait_for_response=True, timeout=120):
        if wait_for_response:
            try:
                ...
                raise CoinsecdCommunicationError(str(e))
